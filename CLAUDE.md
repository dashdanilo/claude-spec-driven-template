...
- `PreToolUse` on Edit/Write: `protect-harness.sh` blocks modifications to the harness's own governance surface — `.claude/settings.json`/`settings.local.json` unconditionally, and `baseline/hooks/*.sh`, `.claude/hooks/*.sh`, `baseline/rules/**`, `.claude/rules/**` only when the edit is cross-repo (a different project reaching into this checkout) — same `.example` exemption
