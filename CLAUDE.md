...
- `orchestrate` - drives a spec's `tasks.md` to completion: reconciles the boxes against the code, classifies each task to select its gates, groups them into **a few cohesive clusters of 5-7 tasks** (one specialist per cluster, running them in order — one agent per task measured worst on every axis), gets approval, then executes **one wave at a time** (whole wave dispatched in a single message, collected, gated once with `verify-before-done` plus the reviewers that wave needs), and opens a PR; halts for a human on anything ambiguous. Portable (stack specialists come from a plugin). See `docs/workflows/feature-pipeline.md`.
