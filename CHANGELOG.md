...
**The harness is installed per person, not per repo** — `baseline/` + `install-global.sh`:
- The machinery moved from `.claude/` to **`baseline/`** at the repo root, and `./install-global.sh` symlinks `baseline/{skills,agents,rules}` into `~/.claude/`. It is then available in **every** project on the machine — njord or not, new or old — with no per-repo setup, no sibling-clone assumption, and no namespacing: `/orchestrate` stays `/orchestrate`. Updating is `git pull` in the checkout, which retires the `install.sh --update` idea entirely.
...
- Hooks cannot ride a directory symlink — they are registered by path — so `install-global.sh` merges them into `~/.claude/settings.json` idempotently, preserving anything already there. `protect-critical.sh` and `check-snapshot-on-session.sh` are **deliberately excluded** from the global set: one knows about lockfiles and migrations, the other about a per-repo snapshot. Both belong to a repo, not to a machine.
- The installer refuses to clobber: if `~/.claude/skills` exists and is not a link into the checkout, it stops and says so rather than destroying files you wrote. `--dry-run`, `--status` and `--uninstall` included.
- `check-index.sh` now scans `baseline/` and `.claude/` for what a repo owns, and `~/.claude/` for **names only** — without that, every consumer repo would report `/orchestrate` as *indexed but not on disk*, and the check sharpened one day earlier would start crying wolf.
- `check-baseline.sh` was rewritten for the model it now lives in: it no longer compares a consumer repo against a central one (there is no consumer repo), it reports whether **your checkout** is behind its remote or carries **uncommitted** edits under `baseline/` — which are live in every project on the machine, unreviewed. It does not fetch, so "up to date" means "as of your last fetch", and it says so.

Rationale, including the cost: ADR 0002 in the njord marketplace. The cost is reach — personal scope is opt-in and machine-local, so a teammate who does not install it does not have it and CI never does. Anything that must hold for everyone stays in the repo.
