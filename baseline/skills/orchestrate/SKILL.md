...
Then run "After opening a PR" in `.claude/rules/harness/git-workflow.md`: a `/orchestrate` run counts as autonomous delivery, so the host's CI monitor goes on without asking when the session has it, and is skipped silently otherwise. Report the PR link and whether the monitor is on.
