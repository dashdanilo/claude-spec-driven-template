...
**[Repomix](https://github.com/yamadashy/repomix)** - packs the entire codebase into a single file. Not this harness's panoramic-context mechanism (that is the built-in repo map, see `docs/decisions/0003-repo-map-over-snapshot.md`, generated by `baseline/scripts/repo-map.sh` with no external dependency); wired only into the manual, opt-in `refresh-snapshot` skill, for handing a single file to some other tool with no filesystem access. Install:
```
